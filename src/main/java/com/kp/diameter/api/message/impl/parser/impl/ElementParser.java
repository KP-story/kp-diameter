...
import com.kp.diameter.api.message.impl.parser.IElementParser;
 ...
 */
public class ElementParser implements IElementParser, Loggable {
    ...
    public byte[] encodeAvpSet(AvpSet avps) {
        ...
        try {
            ...
            e.printStackTrace();
            getLogger().debug("Error during encode avps", e);
        }
    ...
    public byte[] encodeAvp(AvpImpl avp) {
        try {
            ...
            getLogger().debug("Error during encode avp", e);
            return new byte[0];
        }
    }
